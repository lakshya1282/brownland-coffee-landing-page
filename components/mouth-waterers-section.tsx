...
export function MouthWaterersSection() {
  return (
    <section id="dishes" className="py-12 sm:py-16 md:py-20 px-4 sm:px-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="columns-2 sm:columns-2 lg:columns-3 gap-3 sm:gap-4 space-y-3 sm:space-y-4">
          {foodImages.map((image, index) => (
            <div
              key={index}
              className="break-inside-avoid relative overflow-hidden rounded-xl sm:rounded-2xl group cursor-pointer bg-secondary/50 aos-zoom-in"
            ...
            >
              <div className="relative">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-6 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                  <h3 className="text-white text-base sm:text-xl font-bold mb-0.5 sm:mb-1">{image.alt}</h3>
                  <p className="text-white/80 text-xs sm:text-sm line-clamp-2">{image.description}</p>
                </div>
              </div>
            </div>
          ))}
